	...
	*/
	...
	public:
		...
		*/
		...
		/**
		* \brief Operator kopiujπcy.
		*
		* Operator kopiujπcy.
		* \param[in] lista - Lista obiektÛw, ktÛra zostanie skopiowana do obiektu.
		*/
		...
		static bool przenies(const STyp::Identyfikator& klucz, const STyp::Ilosc& ilosc, ListaObiektow<N> & zrodlowa, ListaObiektow<N>& docelowa){
			...
			if (!docelowa.dodaj(obiektPodzielony)){
				if (!zrodlowa.dodaj(obiekt))
					throw STyp::Wyjatek(EXCEPTION_PLACE,STyp::Tekst(), STyp::Tekst(), STyp::Identyfikator(-1), STyp::Tekst("Nieoczekiwany wyjatek"), STyp::Tekst("Wystapi≥ nieoczekiwany wyjatek, ktÛry zaburzy≥ dzia≥anie aplikacji."));
				else
					return false;
			}
			return true;
