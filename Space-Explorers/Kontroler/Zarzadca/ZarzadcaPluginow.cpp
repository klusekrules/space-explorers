...
namespace SpEx{
	...
	ZarzadcaPluginow::ZarzadcaPluginow(const UstawieniaAplikacji& ustawienia, SZmi::ZmianaFabryka& fabryka, SLog::Log& log)
		: fabryka_(fabryka), log_(log), folderPluginow_(ustawienia[ATRYBUT_FOLDER_PLUGINOW])
	{
	}

	const std::list < SumaKontrolnaPliku::SharedPtr > ZarzadcaPluginow::pobierzListeSumKontrolnych() const{
		return lista_;	
	}

	bool ZarzadcaPluginow::zaladujZewnetrzneKlasyZmian(ZarzadcaZasobow & zarzadcaZasobow_){
	...
	bool ZarzadcaPluginow::zaladujDomyslneKlasyZmian(){
		bool rezultat = true;
	...
	std::string ZarzadcaPluginow::napis() const{
		SLog::Logger logger(NAZWAKLASY(ZarzadcaPluginow));
