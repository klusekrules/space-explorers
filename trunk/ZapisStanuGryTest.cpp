...
void ZapisStanuGryTest::zapis(){
	...
	p_a = Aplikacja::getInstance().getGra().getUzytkownik().pobierzPlanete(Identyfikator(1));
...
void ZapisStanuGryTest::odczyt(){
	...
	Planeta p_b = Aplikacja::getInstance().getGra().getUzytkownik().pobierzPlanete(Identyfikator(1));
