	...
	: Info(wezel), przyrostPojemnosciMaksymalnej_(nullptr)
{
	if(wezel){
		try{
			...
			przyrostPojemnosciMaksymalnej_ = Aplikacja::pobierzInstancje().pobierzGre().pobierzFabrykeZmian().Tworz(XmlBO::ZnajdzWezelJezeli<NOTHROW>(wezel,WEZEL_XML_ZMIANA,ATRYBUT_XML_FOR,ATRYBUT_XML_POJEMNOSC_MAKSYMALNA));
