...
void SumaKontrolnaPlikuTest::test()
{
	...
	fclose(fp);
	auto sumaKontrolnaPliku = SpEx::Aplikacja::pobierzInstancje().pobierzZarzadceZasobow().pobierzZasob<SpEx::SumaKontrolnaPliku>(std::string(TYP_ZASOBU_SUMA_KONTROLNA_PLIKU), std::string(PLIK_TESTOWY));
